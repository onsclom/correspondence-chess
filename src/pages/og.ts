...
// Full piece SVG content (matching pieces.ts)
const pieceSVGs: Record<PieceType, { white: string; black: string }> = {
  k: {
    white: `<g fill="none" fill-rule="evenodd" stroke="#000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
    ...
    black: `<g fill="none" fill-rule="evenodd" stroke="#000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
  ...
  q: {
    white: `<g fill="#fff" stroke="#000" stroke-width="1.5" stroke-linejoin="round">
    ...
    black: `<g fill="#000" stroke="#000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
  ...
  r: {
    white: `<g fill="#fff" stroke="#000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
  ...
  b: {
    white: `<g fill="none" stroke="#000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
    ...
    black: `<g fill="none" stroke="#000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
...
function renderPiece(type: PieceType, color: Color, x: number, y: number, size: number): string {
  const scale = size / 45;
  const svgContent = pieceSVGs[type][color === 'w' ? 'white' : 'black'];
...
function generateBoardSVG(board: Board, lastMove: { from: [number, number]; to: [number, number] } | null): string {
  // OG image dimensions (1.91:1 ratio)
  const width = 1200;
  const height = 630;

  // Board sizing - fit nicely within the height with padding
  const squareSize = 70;
  const boardSize = squareSize * 8; // 560px
  const boardX = (width - boardSize) / 2; // Center horizontally
  const boardY = (height - boardSize) / 2; // Center vertically
  ...
  // Background
  svg += `<rect width="${width}" height="${height}" fill="#302e2c"/>`;

  // Board squares (no labels)
  ...
  for (let row = 0; row < 8; row++) {
    for (let col = 0; col < 8; col++) {
      ...
      if (piece) {
        const x = boardX + col * squareSize + squareSize * 0.08;
        const y = boardY + row * squareSize + squareSize * 0.08;
